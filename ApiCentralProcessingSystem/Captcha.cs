...
namespace SV.DocMan.WebApi
{
    public static class Captcha
    {
        ...
        public static bool ValidateCaptchaCode(string userInputCaptcha, List<CaptchaObject> context)
        {
            var isValid = false;
            using (IDbConnection db = new SqlConnection(ConnectionString.Connection))
            {
                ...
                var result = db.Query<int>("HT_CaptChaValidator", parameters, null, true, null, CommandType.StoredProcedure).FirstOrDefault();
                if (result > 0)
                {
                    isValid = true;
                }
        ...
        public static void InsertCaptcha(string captcha)
        {
            try
            {
                using (IDbConnection db = new SqlConnection(ConnectionString.Connection))
                {
                    ...
                    db.Execute("Ht_CaptChaInsertData", parameters, commandType: CommandType.StoredProcedure);
                }
            }
            catch (Exception ex)
            {
        ...
        public static void DeleteCaptcha(string captcha)
        {
            using (IDbConnection db = new SqlConnection(ConnectionString.Connection))
            {
                DynamicParameters parameters = new DynamicParameters();
                parameters.Add("@Code", captcha);
                db.Execute("HT_CaptChaDelete", parameters, commandType: CommandType.StoredProcedure);
            }
        }
